...
| **연필** | 종이 질감 · 스케치 상자 | 66KB(gzip) | **선택 시 지연 로드** |
| **게임** | 픽셀(인광관) · 도트 폰트 · 스캔라인 | 10KB(gzip) | **선택 시 지연 로드** |

### 지연 로드

연필 테마는 종이 질감을 인라인 SVG 로 그려 66KB(gzip) 다.
대부분의 사용자가 쓰지 않을 것을 첫 로드에 태우지 않는다.

- 라이트·다크는 `app.css` 번들에 포함 (합쳐 4KB)
- 연필·게임은 `/themes/*.css` 로 따로 두고 고를 때 `<link>` 를 주입
- 도착 전까지 `data-theme-loading` 으로 화면을 숨겨 깜빡임을 막는다
...
`dark.css` 는 `:root, [data-theme="dark"]` 로 devkanban 의 **기본 테마** 역할을 겸한다.
라이트를 먼저 임포트하면 dark 의 `:root` 가 뒤에서 덮어써 **라이트가 다크로 보인다.**

```css
@import "./themes/dark.css";   /* 반드시 먼저 */
@import "./themes/light.css";
@import "./accent.css";        /* 액센트 재정의는 맨 뒤 */
```
...
`backend/assets/css/tokens.css` 가 이름 목록이자 라이트 기본값이다.
테마 파일이 `[data-theme="…"]` 로 덮는다.
